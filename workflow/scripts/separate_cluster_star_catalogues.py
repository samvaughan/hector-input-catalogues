...
for (region, row), standard_filename, guide_filename in zip(region_information.iterrows(), output_standard_star_cats, output_guide_star_cats):
    ...
    master_guide_stars.loc[guide_mask, columns_in_order].to_csv(guide_filename, index=False)
    ...
    master_standard_stars.loc[standard_mask, columns_in_order].to_csv(standard_filename, index=False)
