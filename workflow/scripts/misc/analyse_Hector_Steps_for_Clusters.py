"""
Investigate how the Hector steps affect each cluster
"""
...
cluster_info = pd.read_table(
    "resources/ClusterInformation/HectorCluster_Information.txt",
...
for ax, cluster in zip(axs.ravel(), clusters):
    if cluster == "A3391/A3395":
        ax.scatter(
            df.loc[df["NearClus"].isin(["A3391", "A3395"]), "z"],
            df.loc[df.NearClus.isin(["A3391", "A3395"]), "logMstarProxy_LS"],
            label="A3391/A33945",
        )
        ax.axvline(cluster_info.loc["A3395", "z"], c="k", linestyle="dashed")
    else:
        ax.scatter(
            df.loc[df["NearClus"] == cluster, "z"],
            df.loc[df.NearClus == cluster, "logMstarProxy_LS"],
            label=cluster,
        )
        ax.axvline(cluster_info.loc[cluster, "z"], c="k", linestyle="dashed")
